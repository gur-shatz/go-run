...
// ConsoleParser parses common structured console logs.
type ConsoleParser struct{}

// StartsEntryLine reports whether a physical line starts a console log entry.
func (ConsoleParser) StartsEntryLine(line []byte) bool {
...
func (ConsoleParser) IgnoreLine(line []byte) bool {
	return bytes.HasPrefix(bytes.TrimSpace(line), []byte("===="))
}

// ParseLine parses one already-decoded log line.
func (ConsoleParser) ParseLine(line []byte, meta LineMeta) (Entry, bool) {
...
func (ConsoleParser) Match(entry Entry, filter Filter) bool {
